class Ui {
  ...
  showCalculatedTip(solution) {
    ...
    this.totalPercentage.textContent = `$${solution.amountPerPerson.toFixed(
      2
    )}`;
    document.querySelector(".calculated-tip").style.display = "none";
    document.querySelector(".loading-spinner-blocks").style.display =
      "inline-block";
    setTimeout(this.loading, 2000);
  }
  resetUi(billInput, customInput, personInput, tipBtn) {
    billInput.value = "";
    customInput.value = "";
    personInput.value = "";
